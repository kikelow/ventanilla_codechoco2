...
$s = "SELECT apoyo.id,apoyo.descripcion,apoyo.nombre_entidad,apoyo.tipo_entidad_id,apoyo.año FROM `apoyo`
INNER JOIN informacion_complementaria ON informacion_complementaria.id = apoyo.info_com_id
WHERE informacion_complementaria.empresa_id = '$empresa'";
$r = mysqli_query($conn,$s);
while ($rw = mysqli_fetch_assoc($r)) {
 echo " <div class='row'>
 <div class='input-field col s12 m4 l4'>
 <input type='text' id=''  name='apoyo_descripcion[]' value='$rw[descripcion]'/>
 <label for='' class='active'>Descripción</label>
 <input type='hidden' id='' name='apoyo_hidden[]' value='$rw[id]'>
 </div>
 <div class='input-field col s12 m4 l4'>
 <input type='text' id=''  name='apoyo_entidad[]' value='$rw[nombre_entidad]'/>
 ...
 if(mysqli_num_rows($r1)>0){
  while($result1=mysqli_fetch_assoc($r1)){
    if ($rw['tipo_entidad_id'] == $result1['id']) {
     echo"<option value=".$result1['id']." selected='selected'>".$result1['nombre' ]."</option>";
   }else{
    echo"<option value=".$result1['id'].">".$result1['nombre' ]."</option>";
  }
...
<div class='input-field col s12 m2 l2'>
<input type='number' id=''  name='apoyo_anio[]' maxlength='4' value='$rw[año]' />
<label for='' class='active'>Año</label>
</div>
</div>";
}
$num_familias = 0;
$s="SELECT num_familias FROM informacion_complementaria WHERE empresa_id = '$empresa'";
$r = mysqli_query($conn,$s);
while ($rw = mysqli_fetch_assoc($r)) {
  $num_familias = $rw['num_familias'];
}
...
<input type='number' id='num_familias'  name='num_familias' value='$num_familias' />
<label for='num_familias' class='active'>¿Cúantas familias se benefician de la actividad del negocio, bien sea por generación de ingresos o por incentivos recibidos?</label>
</div>
</div>";
echo "</span></div></li>";
...
</div> <div id='cadena_contenido'>";

$i = 0;
$s="SELECT cadena_valor.pregunta_id as id, pregunta_indicativa.descripcion,cadena_valor.respuesta_id FROM `cadena_valor` 
...
WHERE pregunta_indicativa.aspecto_id = 25 AND informacion_complementaria.empresa_id = '$empresa' AND cadena_valor.pregunta_id != 136 order by cadena_valor.pregunta_id";
...
$i = 0;
$s="SELECT sost_economica.id, bienes_servicios.nombre,bienes_servicios.lider, sost_economica.u_vendidadas_anuales,sost_economica.unidad_medida_id,sost_economica.cantidad_unidad,sost_economica.costo_produccion_unidad,sost_economica.precio_v_unitario,sost_economica.ganancia_unidad,sost_economica.ventas_anual,sost_economica.ingreso_superior 
FROM `sost_economica` 
INNER JOIN informacion_complementaria ON informacion_complementaria.id = sost_economica.info_com_id
INNER JOIN bienes_servicios ON bienes_servicios.id = sost_economica.bien_servicio_id 
WHERE informacion_complementaria.empresa_id = '$empresa'";
...
if(mysqli_num_rows($r)>0){
  while($rw=mysqli_fetch_assoc($r)){
   ...
   <input type='hidden' name='sost_hidden[]' id='sost_hidden' value='$rw[id]'>
   </div>
   <div class='input-field col s12 m3 l3'>
   <input   type='number' name='unidad_v_anual[]' id='unidad_v_anual".$i."' class='aut' value='$rw[u_vendidadas_anuales]'/>
   <label for='unidad_v_anual".$i."' class='active'>unidades vendidas anual</label>
   </div>

   <div class='input-field col s12 m2 l2'>
   <select  name='unidad_medida[]' id='unidad_medida".$i."'>";
   $s1="select id,nombre from unidad_medida ";
   $r1= mysqli_query($conn,$s1) or die('Error');
   if(mysqli_num_rows($r)>0){
    while($result=mysqli_fetch_assoc($r1)){
      if ($rw['unidad_medida_id'] == $result['id']) {
        echo "<option value=".$result['id']." selected='selected'>".$result['nombre']."</option>";
      }else{
  ...
  <label for='costo_pro_unidad".$i."' class='active'>Costo producción unidad</label>
  </div>
  <div class='input-field col s12 m2 l2'>
  <input   type='number' name='precio_v_unitario[]' id='precio_v_unitario".$i."' class='aut' value='$rw[precio_v_unitario]' />
  <label for='precio_v_unitario".$i."' class='active'>Precio venta unitario</label>
  </div>
  <div class='input-field col s12 m2 l2'>
  <input readonly  type='number' name='ganancia_unidad[]' id='ganancia_unidad".$i."' value='$rw[ganancia_unidad]'/>
  <label for='ganancia_unidad".$i."' class='activar'>Ganacias por unidad</label>
  </div>
  <div class='input-field col s12 m2 l2'>
  <input readonly  type='number' name='venta_anual[]' id='venta_anual".$i."' value='$rw[ventas_anual]' />
  ...
  if(mysqli_num_rows($r2)>0){
    while($result2=mysqli_fetch_assoc($r2)){
      if ($rw['ingreso_superior'] == $result2['id']) {
        echo "<option value=".$result2['id']." selected='selected'>".$result2['nombre' ]."</option>";
      }else{
...
$venta_total_anual = 0;
$ingresos_superior = 0;
$s="SELECT bienes_servicios_adicionales.bien,bienes_servicios_adicionales.costo_total_produccion,bienes_servicios_adicionales.venta_total_anual,bienes_servicios_adicionales.ingresos_superior 
FROM `bienes_servicios_adicionales`
INNER JOIN informacion_complementaria ON informacion_complementaria.id = bienes_servicios_adicionales.info_com_id
WHERE informacion_complementaria.empresa_id = '$empresa'";
...
if(mysqli_num_rows($r)>0){
  while($rw=mysqli_fetch_assoc($r)){
    $bien = $rw['bien'];
    $costo_total_produccion = $rw['costo_total_produccion'];
    $venta_total_anual = $rw['venta_total_anual'];
    $ingresos_superior = $rw['ingresos_superior'];
  }
}
echo "<div class='row'>
<div class='input-field col s12 m4 l4'>
<input type='text' id='otro_bien_nom'  name='otro_bien_nom' value='$bien'   />
<label for='' class='active'>Otros bienes o servicios</label>
</div>
<div class='input-field col s12 m3 l3'>
<input type='number' id='otro_t_produccion'  name='otro_t_produccion' value='$costo_total_produccion'  />
<label for='' class='active'>Costo total producción</label>
</div>
<div class='input-field col s12 m2 l2'>
<input type='number' id='otro_venta_total' class='aut'  name='otro_venta_total' value='$venta_total_anual'  />
<label for='' class='active'>Ventas totales anual</label>
</div>
...
if(mysqli_num_rows($r2)>0){
  while($result2=mysqli_fetch_assoc($r2)){
    if ($ingresos_superior == $result2['id']) {
      echo"<option value=".$result2['id']." selected='selected'>".$result2['nombre' ]."</option>";
    }else{
      echo"<option value=".$result2['id'].">".$result2['nombre' ]."</option>";
    }
...
$obra_total = 0;
$total_ventas = 0;
$s="SELECT total_ventas.costo_pro_insumos_totales, total_ventas.costo_pro_mano_obra,total_ventas.total_ventas_realizadas_ant FROM `total_ventas`
INNER JOIN informacion_complementaria ON informacion_complementaria.id = total_ventas.info_com_id
WHERE informacion_complementaria.empresa_id = '$empresa'";

$r= mysqli_query($conn,$s) or die(mysqli_error($conn));
if(mysqli_num_rows($r)>0){
  while($rw=mysqli_fetch_assoc($r)){
    $insumos_total = $rw['costo_pro_insumos_totales'];
    $obra_total = $rw['costo_pro_mano_obra'];
    $total_ventas = $rw['total_ventas_realizadas_ant'];
  }
}
...
<div class='row' style='margin-bottom: 0px'>
<div class='input-field col s12 m12 l12'>
<input id='costo_p_insumos' name='costo_p_insumos' type='number' value='$insumos_total'> 
<label for='costo_p_insumos' class='active'>Valor anual</label>
</div>
</div>
</div>
...
<div class='row' style='margin-bottom: 0px'>
<div class='input-field col s12 m12 l12'>
<input id='costo_p_mano_obra' name='costo_p_mano_obra' type='number' value='$obra_total' >
<label for='costo_p_mano_obra' class='active'>Valor anual</label>
</div>
</div>
</div>
...
<div class='row' style='margin-bottom: 0px'>
<div class='input-field col s12 m12 l12'>
<input id='total_ventas_realizadas' name='total_ventas_realizadas' type='number' value='$total_ventas'>
...
$i = 0;
$s="SELECT comercializacion.pregunta_id as id, pregunta_indicativa.descripcion,comercializacion.numero,comercializacion.local,comercializacion.regional,comercializacion.nacional,comercializacion.global,comercializacion.observacion 
FROM `comercializacion` 
INNER JOIN informacion_complementaria ON comercializacion.info_com_id = informacion_complementaria.id
INNER JOIN pregunta_indicativa ON pregunta_indicativa.id = comercializacion.pregunta_id 
WHERE pregunta_indicativa.aspecto_id = 27 AND informacion_complementaria.empresa_id = '$empresa' 
order by comercializacion.pregunta_id";
...
if(mysqli_num_rows($r)>0){
  while($rw=mysqli_fetch_assoc($r)){
   ...
   <input type='number' id='comer_numero".$i."' name='comer_numero[]' value='$rw[numero]'/>
   <label for='comer_numero".$i."' class='active'>Número</label>
   </div> 
   <div class='input-field col s12 m2 l1'>
   <input type='number' id='comer_local".$i."' name='comer_local[]' value='$rw[local]'/>
   <label for='comer_local".$i."' class='active'>Local</label>
   </div> 
   <div class='input-field col s12 m2 l1'>
   <input type='number' id='comer_regional".$i."' name='comer_regional[]' value='$rw[regional]'/>
   <label for='comer_regional".$i."' class='active'>Regional</label>
   </div> 
   <div class='input-field col s12 m2 l1'>
   <input type='number' id='comer_nacional".$i."' name='comer_nacional[]' value='$rw[nacional]'/>
   <label for='comer_nacional".$i."' class='active'>Nacional</label>
   </div> 
   <div class='input-field col s12 m2 l1'>
   <input type='number' id='comer_global".$i."' name='comer_global[]' value='$rw[global]'/>
   <label for='comer_global".$i."' class='active'>Global</label>
   </div> 
   <div class='input-field col s12 m12 l5'>
   <input type='text' id='comer_observacion".$i."' name='comer_observacion[]' value='$rw[observacion]'/>
   <label for='comer_observacion".$i."' class='active'>Observación</label>
   </div> 
...
</div> <div id='turismo_contenido'>";

$i = 0;
$s="SELECT turismo.pregunta_id as id,pregunta_indicativa.descripcion,turismo.respuesta_id FROM `turismo` 
INNER JOIN informacion_complementaria ON turismo.info_com_id = informacion_complementaria.id
INNER JOIN pregunta_indicativa ON pregunta_indicativa.id = turismo.pregunta_id 
WHERE pregunta_indicativa.aspecto_id = 28 AND informacion_complementaria.empresa_id = '$empresa'
order by turismo.pregunta_id";
